...
void SoftClip::processBlock(juce::dsp::AudioBlock<float>& block) noexcept {
    ...
    {
        float saturationAmount = powf(saturationKnob.getNextValue(0) * 0.01f, 1.5f) * 100.0f * 0.24 + 0.00000001f;
        float outGain = juce::Decibels::decibelsToGain(juce::jmap(( 1.0f - saturationAmount * 0.05f), -6.5f, 0.0f));

        for (int channel = 0; channel < block.getNumChannels(); channel++)
        {
            float xn = block.getSample(channel, sample);
            block.setSample(channel, sample, approxTanhWaveshaper2(xn, saturationAmount) * outGain);
        }
    }
}