...
template <typename SampleType>
inline SampleType SVTPTFilter<SampleType>::processSamplePeakFilter (int channel, SampleType inputValue, SampleType peakGain)
{
    auto& ls1 = s1[(size_t) channel];
    auto& ls2 = s2[(size_t) channel];
    ...
    float A = std::sqrt(peakGain);

    auto yPeak = yLP + yHP + R2 * A * yBP;
